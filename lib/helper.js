...
function isGeohashWithinRangeBox(geohash, rangeBox) {
    ...
    if (rangeBox.MinLongitude <= geobox[0][1] // top left lon
    &&
    rangeBox.MaxLongitude >= geobox[1][1] // top right lon
    &&
    rangeBox.MinLatitude <= geobox[3][0] // bottom left lat
    &&
    rangeBox.MaxLatitude >= geobox[0][0]) // top left lat
    {
        return true;
    }
    else {
...
function isPointWithinBox(x, y, x1, y1, x2, y4) {
    ...
    if (x1 <= x // top left longitude
    &&
    x2 >= x // top right longitude
    &&
    y4 <= y // bottom left latitude
    &&
    y1 >= y) // top left latitude
    {
        return true;
    }
    else {
